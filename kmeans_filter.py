...
def get_topk_colors_kmeans(img_np, k, num_clusters=100, random_state=42):
...
def plot_filtered_and_original(
    ...
    ax4.imshow(
        color_palette[:, :, ::-1].astype(np.uint8), aspect=1/5
    )
    ax4.set_axis_off()
    plt.tight_layout()
...
def main(
    ...
    top_k_colors = get_topk_colors_kmeans(
        img_np, n_colors, num_clusters=num_clusters, random_state=random_state
    )
    ...
    if ref_path is not None:
        ...
        top_k_colors_ref = get_topk_colors_kmeans(
            ref_np, n_colors, num_clusters=num_clusters, random_state=random_state
        )
